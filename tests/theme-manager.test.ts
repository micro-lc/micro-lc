...
import type { MlcAntdThemeManager } from '../packages/layout/dist/types/web-components/mlc-antd-theme-manager'

test('should send configuration to micro-lc API with default values', async ({ page }) => {
  await page.goto('http://localhost:3000/pages/')

  await page.evaluate(() => {
    const src = '/packages/layout/dist/mlc-antd-theme-manager.js'
    ...
    const microlcApi: Partial<MlcAntdThemeManager['microlcApi']> = {
      getExtensions: () => ({ css: { setStyle: (st) => { style = st } } }),
    }
    window.customElements.whenDefined('mlc-antd-theme-manager').then(() => {
      const manager = document.createElement('mlc-antd-theme-manager') as MlcAntdThemeManager
      manager.microlcApi = microlcApi
      document.body.appendChild(manager)
    }).catch(console.error)
    Object.defineProperty(window, 'microlcStyle', { get: () => style })
  })
  ...
  await page.waitForFunction('window.microlcStyle !== undefined')
  const style = await page.evaluate('window.microlcStyle')
  expect(style).toEqual({ global: expectedVars, nodes: {} })
})
  ...
  await page.evaluate(() => {
    ...
    window.customElements.whenDefined('mlc-antd-theme-manager').then(() => {
      const manager = Object.assign(
        document.createElement('mlc-antd-theme-manager') as MlcAntdThemeManager,
        {
          errorColor: '#FCFF4D',
